...
Executable toy graph and trace checks live in `Examples/BFS.lean`.
...
`ResourceModel.lean`. `Correctness.lean` proves exact reachability of the completed
Boolean-table run and finite connected shortest-path-tree certificates. `Complexity.lean` proves
one cost-independent BFS operation profile, then reuses it for arbitrary exact costs satisfying
explicit upper bounds and for the traditional unit-cost corollaries. Run the executable example
