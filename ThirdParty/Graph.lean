...
This is a local copy of the graph API proposed in CSLib PR #503. It remains experimental until
that API is merged upstream; resource-aware algorithms should depend on `Interface.lean` instead.
...
namespace ResourceAware.Graph.Concrete
variable {α β : Type*}
...
end ResourceAware.Graph.Concrete
