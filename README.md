...
Write the algorithm once. Interpret it under independently varying semantic and
cost models — and prove the models do not disturb each other. Algorithms are
written as free programs over an abstract operation signature; semantic backends
and cost models are supplied separately, and a generic interpreter proves that
swapping the cost model preserves the returned value, the final state, and the
erased operation trace. Ordered operation traces then let one execution answer
many cost questions, from exact modeled cost through to asymptotic
(`IsBigO`) bounds.
...
are CSLib's, and `ThirdParty/Graph.lean` is a local copy of the graph API
proposed in CSLib PR #503 by its listed authors. See `NOTICE` for attribution.
...
models with a proved independence theorem, on ordered measured traces with
erasure and reweighting, and on graph algorithms with representation-sensitive
cost models; AlgoLean covers ground this project does not, including
